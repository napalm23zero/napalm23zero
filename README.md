<div align="center">

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- BANNER — minimalist, Cinzel medium, dried-blood red, no stroke/glow     -->
<!-- The typing speed is set high (vCenter, single line) so the title        -->
<!-- renders as static after one quick stroke on first paint.                -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

<img src="https://readme-typing-svg.demolab.com?font=Cinzel&weight=600&size=56&duration=1&pause=99999999&color=9D2933&center=true&vCenter=true&width=900&height=80&letterSpacing=0.35em&lines=RODRIGO+DANTAS" alt="Rodrigo Dantas"/>

<img src="https://readme-typing-svg.demolab.com?font=Cinzel&weight=500&size=14&duration=1&pause=99999999&color=D4D4D4&center=true&vCenter=true&width=700&height=30&letterSpacing=0.6em&lines=STAFF+ENGINEER++%C2%B7++ARCHITECT++%C2%B7++FOUNDER" alt="Staff Engineer · Architect · Founder"/>

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=400&size=15&duration=3500&pause=900&color=A8A29E&center=true&vCenter=true&width=820&height=30&lines=Peace+is+a+lie.+There+is+only+the+codebase.;Two+senior+contracts+in+parallel.+One+profitable+SaaS+of+my+own.;Open+to+co-founder+%2F+CTO+%2F+Staff+Engineer+roles." alt="Tagline"/>

[![Profile Views](https://komarev.com/ghpvc/?username=napalm23zero&style=flat-square&color=9D2933&label=profile+views&labelColor=0a0606)](https://github.com/napalm23zero)
[![Followers](https://img.shields.io/github/followers/napalm23zero?style=flat-square&color=9D2933&labelColor=0a0606&label=followers)](https://github.com/napalm23zero)
[![Years](https://img.shields.io/badge/15%2B-years%20shipping-9D2933?style=flat-square&labelColor=0a0606)](#)
[![Status](https://img.shields.io/badge/status-seeking%20a%20worthy%20alliance-9D2933?style=flat-square&labelColor=0a0606)](#-an-offer-of-alliance-)

</div>

---

## `⚔ THE RECORD ⚔`

Senior software engineer, architect, and founder. Building production systems since 2009 across fintech, banking, retail, travel, tax compliance, and SaaS — most recently in parallel for Thomson Reuters, FiServ, and a B2B SaaS of my own.

| Where | When | What was built |
|---|---|---|
| **Thomson Reuters** *(via SinQia)* | 2025 → now | **SCANC** — petrochemical fiscal-obligation engine. CDC + ETL in Python/Pandas over Firebird, Oracle PL/SQL SAFX rules, AWS Lambda/SQS/SNS event orchestration. End-to-end run under **12 seconds**. |
| **FiServ** *(via Go2Win)* | 2024 → now | **AI-powered incident intelligence** for banking compliance. LLM agents generate Splunk queries against curated RCA history; most recurring incidents now resolve near-automatically. Full-stack investigation: Apigee → NGINX → OpenShift/PCF → Java APIs → VMX mainframe. |
| **HustleTech** *(founder)* | 2019 → now | **Profitable B2B SaaS**, architected from zero. Java/Spring microservices on GCP, React + React Native, PostgreSQL, Redis, Terraform, GitHub Actions. Dozens of paying clients. Tens of thousands of active users. 5-person team. |
| **KIS Solutions** | 2022 → 2024 | Technical direction across a team of 8. CDC and ETL pipelines (Node/Next), mobile and web delivery for multiple US clients. |
| **Casas Bahia** *(Via Varejo)* | 2020 → 2021 | Tech Lead. Microservices migration with clean architecture. Multi-database systems: Oracle, MongoDB, Redis, DB2, Azure. CI/CD overhaul that actually shipped. |
| **Banco BV** + **Opah IT** | 2021 → 2022 | Java/Spring backends and Apache Camel integrations for one of Brazil's largest private banks. Reliability work that held under real traffic. |
| **UOL** | 2019 → 2020 | Java 8 + Kotlin services with Quarkus, Apache Camel, TDD with Spock. |
| **CVC Corp** | 2019 | Travel-industry Spring microservices at scale; Kafka, ELK, Jenkins, Oracle, MongoDB. |

Earlier work at **Samsung**, **LG CNS**, **Philips**, and **Atos** (Rio 2016 Olympics). Full history in [The Chronicles](#-chronicles-of-the-journey-) below.

---

- **B.Sc. Computer Science**, Universidade Estácio — `2023 → 2026`
- **Conference speaker** — *Best Practices in RESTful API Development* (2020) · *MongoDB: What Problems Does It Actually Solve?* (2021)
- **Three working languages** — Portuguese (native), English (fluent), Spanish (advanced); meetings, interviews, and negotiations run in all three
- **GMT-3** — overlaps US Eastern fully, US Pacific mostly; remote-first since 2019

---

Open to a **co-founder**, **CTO / VP of Engineering**, or **Staff / Principal Engineer** role at an early- to growth-stage company where architecture, technical direction, and product depth matter more than headcount.

Reachable at [rodrigo.dantas@hustletech.dev](mailto:rodrigo.dantas@hustletech.dev) · [30-minute call](https://calendar.app.google/ovSnBAqvXtRztvsd7).

---

## `⛧ THREE FRONTS · ONE ARCHITECT ⛧`

> Yes, **two senior engineering contracts in parallel** plus running my own profitable SaaS.
> The empire was not built by people who needed eight hours of sleep.

<table>
<tr>
<td width="33%" align="center" valign="top">

### 🩸
**Senior SWE @ FiServ**
*(via Go2Win)*
`Oct 2024 → Now`

Built an **AI-powered incident intelligence system** — LLM agents craft Splunk incantations, cross-reference past failures, and resolve recurring incidents near-automatically.

`Apigee · OpenShift · Java · VMX`

</td>
<td width="33%" align="center" valign="top">

### 🔥
**Senior SWE @ Thomson Reuters**
*(via SinQia)*
`Feb 2025 → Now`

Built **SCANC from scratch** — centralized fiscal obligation engine. CDC + ETL pipelines processing millions of records end-to-end in **under 12 seconds**.

`Python · Pandas · Oracle PL/SQL · AWS`

</td>
<td width="33%" align="center" valign="top">

### ⚡
**Founder @ HustleTech**
*Profitable B2B SaaS · my own empire*
`Nov 2019 → Now`

Architected from zero. **Dozens of paying clients, tens of thousands of users.** A 5-person crew. AI agents handle the toil — humans handle the strategy.

`Spring · React · GCP · Terraform`

</td>
</tr>
</table>

---

## `⚔ THE GIFTS I BRING TO THE TABLE ⚔`

<table>
<tr>
<td width="50%" valign="top">

### 🏛️ **Architecture That Outlives Its Maker**
Microservices, event-driven systems, CDC/ETL pipelines, multi-tenant SaaS. I've built systems that survived Black Friday and Brazilian fiscal compliance. Your peak traffic does not scare me — *it merely confirms the design*.

### 👥 **Command Without Cruelty**
Led teams at Casas Bahia, Opah, Banco BV, KIS. I turn juniors into senior engineers — not by fear, but by clarity. Strong opinions, loosely held; disagree, then commit.

### 🤖 **AI-Native Engineering**
Production LLM agent systems — not chatbot toys. Incident response, code review, vulnerability scanning, automated testing. The kind that actually works in prod, where the cost of failure is real.

</td>
<td width="50%" valign="top">

### ⚔️ **Polyglot Pragmatism**
Java, Python, TS/Node, Go, Kotlin, C#. *The right blade for the right wound* — not the language I tweet about. 8+ stacks shipped to production.

### 🌍 **A Voice in Three Tongues**
Fluent English, advanced Spanish, native Portuguese. Worked with teams in the US, Europe, Mexico, Argentina, Colombia. I speak engineer **and** stakeholder — and I bridge between them when neither will.

### 🎤 **Public Technical Voice**
Conference speaker (RESTful APIs · MongoDB architecture). Currently bootstrapping a public writing presence — *the next phase of the doctrine*.

</td>
</tr>
</table>

---

## `⛧ THE ARSENAL ⛧`

<div align="center">

#### 🗡️ Weapons of Choice 🗡️

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" width="48" height="48" alt="Java" title="Java"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="48" height="48" alt="Python" title="Python"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" width="48" height="48" alt="TypeScript" title="TypeScript"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="48" height="48" alt="JavaScript" title="JavaScript"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" width="48" height="48" alt="Node.js" title="Node.js"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/go/go-original-wordmark.svg" width="48" height="48" alt="Go" title="Go"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kotlin/kotlin-original.svg" width="48" height="48" alt="Kotlin" title="Kotlin"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" width="48" height="48" alt="C#" title="C#"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" width="48" height="48" alt="C++" title="C++"/>

<br><br>

#### 🛡️ Frameworks Forged in Production 🛡️

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/spring/spring-original.svg" width="48" height="48" alt="Spring" title="Spring"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/quarkus/quarkus-original.svg" width="48" height="48" alt="Quarkus" title="Quarkus"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nestjs/nestjs-original.svg" width="48" height="48" alt="NestJS" title="NestJS"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" width="48" height="48" alt="React" title="React"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/angularjs/angularjs-original.svg" width="48" height="48" alt="Angular" title="Angular"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" width="48" height="48" alt="Next.js" title="Next.js"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/fastapi/fastapi-original.svg" width="48" height="48" alt="FastAPI" title="FastAPI"/>

<br><br>

#### 🏰 Citadels & Infrastructure 🏰

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/googlecloud/googlecloud-original.svg" width="48" height="48" alt="GCP" title="Google Cloud"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" width="48" height="48" alt="AWS" title="AWS"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/azure/azure-original.svg" width="48" height="48" alt="Azure" title="Azure"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" width="48" height="48" alt="Docker" title="Docker"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kubernetes/kubernetes-plain.svg" width="48" height="48" alt="Kubernetes" title="Kubernetes"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/terraform/terraform-original.svg" width="48" height="48" alt="Terraform" title="Terraform"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jenkins/jenkins-original.svg" width="48" height="48" alt="Jenkins" title="Jenkins"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/githubactions/githubactions-original.svg" width="48" height="48" alt="GitHub Actions" title="GitHub Actions"/>&nbsp;
<img src="https://cdn.simpleicons.org/nginx/009639" width="48" height="48" alt="NGINX" title="NGINX"/>&nbsp;
<img src="https://cdn.simpleicons.org/redhatopenshift/EE0000" width="48" height="48" alt="OpenShift" title="Red Hat OpenShift"/>

<br><br>

#### 💾 Vaults of Memory 💾

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="48" height="48" alt="PostgreSQL" title="PostgreSQL"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" width="48" height="48" alt="MongoDB" title="MongoDB"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/redis/redis-original.svg" width="48" height="48" alt="Redis" title="Redis"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" width="48" height="48" alt="MySQL" title="MySQL"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/oracle/oracle-original.svg" width="48" height="48" alt="Oracle / PL/SQL" title="Oracle / PL/SQL"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/microsoftsqlserver/microsoftsqlserver-original.svg" width="48" height="48" alt="SQL Server" title="SQL Server"/>&nbsp;
<img src="https://dbdb.io/media/logos/firebird.png" width="48" height="48" alt="Firebird" title="Firebird"/>&nbsp;
<img src="https://discover.strongdm.com/hubfs/ibm-db2-logo.png" width="48" height="48" alt="IBM DB2" title="IBM DB2"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/apachekafka/apachekafka-original.svg" width="48" height="48" alt="Apache Kafka" title="Apache Kafka"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/rabbitmq/rabbitmq-original.svg" width="48" height="48" alt="RabbitMQ" title="RabbitMQ"/>&nbsp;
<img src="https://cdn.simpleicons.org/apache/D22128" width="48" height="48" alt="Apache Camel" title="Apache Camel"/>

<br><br>

#### 🧠 The Dark Arts · AI & LLM 🧠

<img src="https://static.freepnglogo.com/images/all_img/1702059841openai-icon-png.png" width="48" height="48" alt="OpenAI" title="OpenAI / LLM Agents"/>&nbsp;
<img src="https://cdn.simpleicons.org/anthropic/D97757" width="48" height="48" alt="Anthropic" title="Anthropic / Claude"/>&nbsp;
<img src="https://cdn.simpleicons.org/langchain/1C3C3C" width="48" height="48" alt="LangChain" title="LangChain / RAG"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" width="48" height="48" alt="Pandas" title="Pandas"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/numpy/numpy-original.svg" width="48" height="48" alt="NumPy" title="NumPy"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg" width="48" height="48" alt="TensorFlow" title="TensorFlow"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pytorch/pytorch-original.svg" width="48" height="48" alt="PyTorch" title="PyTorch"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/elasticsearch/elasticsearch-original.svg" width="48" height="48" alt="Elasticsearch" title="Vector DBs (Elasticsearch)"/>

<br><br>

#### 🐧 Battle Stations 🐧

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" width="48" height="48" alt="Linux" title="Linux"/>&nbsp;
<img src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/9e/UbuntuCoF.svg/3840px-UbuntuCoF.svg.png?utm_source=commons.wikimedia.org&utm_campaign=index&utm_content=thumbnail" width="48" height="48" alt="Ubuntu" title="Ubuntu"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/archlinux/archlinux-original.svg" width="48" height="48" alt="Arch Linux" title="Arch Linux"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/debian/debian-original.svg" width="48" height="48" alt="Debian" title="Debian"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/raspberrypi/raspberrypi-original.svg" width="48" height="48" alt="Raspberry Pi" title="Raspberry Pi"/>&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/arduino/arduino-original.svg" width="48" height="48" alt="Arduino" title="Arduino / ESP32"/>

</div>

---

## `⚡ ARCHIVES OF THE ARCHITECT ⚡`

<div align="center">

<a href="https://github.com/napalm23zero">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=napalm23zero&show_icons=true&theme=radical&include_all_commits=true&count_private=true&hide_border=true&bg_color=0a0606&title_color=9D2933&icon_color=9D2933&text_color=d4d4d4&ring_color=9D2933" alt="GitHub stats"/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=napalm23zero&layout=compact&theme=radical&hide_border=true&bg_color=0a0606&title_color=9D2933&text_color=d4d4d4&langs_count=10" alt="Top languages"/>
</a>

<br><br>

<a href="https://git.io/streak-stats">
  <img src="https://streak-stats.demolab.com?user=napalm23zero&theme=radical&hide_border=true&background=0a0606&ring=9D2933&fire=9D2933&currStreakLabel=9D2933&sideLabels=d4d4d4&dates=a8a29e&stroke=2a0e0e&currStreakNum=d4d4d4&sideNums=d4d4d4" alt="GitHub streak"/>
</a>

</div>

---

## `🗡️ ARTIFACTS IN FORGING 🗡️`

> 🧪 **The forge is hot.** A public portfolio is being shaped. First strikes within 60 days.

<table>
<tr>
<td width="50%" valign="top">

### 🏛️ Distributed Systems Playground
*(in forging)*

Reference architectures for the patterns I keep rebuilding: **event sourcing, CQRS, saga orchestration**. Production-grade. Tests, benchmarks, docs — *as the doctrine demands*.

`Java · Spring · Kafka · Postgres`

---

### ⚙️ Opinionated Backend Starter
*(coming soon)*

The template I wish I had a decade ago. Auth, observability, CI/CD, multi-tenancy — *all that a young apprentice should not have to learn the hard way*.

`NestJS · TypeScript · Docker · GCP`

</td>
<td width="50%" valign="top">

### 🧠 LLM Agent Reference Stack
*(coming soon)*

Production patterns for LLM agents that survive contact with reality. Tool calling, evals, fallback strategies, cost controls. *Demos are not deployments.*

`Python · FastAPI · LangChain · Vector DBs`

---

### 📡 Edge + Cloud IoT Stack
*(planned)*

Because most "edge computing" tutorials end at *"blink an LED."* Mine will not.

`ESP32 · Python · GCP IoT · MQTT`

</td>
</tr>
</table>

#### Already in the wild:

[![NestJS Template](https://github-readme-stats.vercel.app/api/pin/?username=napalm23zero&repo=ultimate-typescript-nestjs-api-template&theme=radical&hide_border=true&bg_color=0a0606&title_color=9D2933&icon_color=9D2933&text_color=d4d4d4)](https://github.com/napalm23zero/ultimate-typescript-nestjs-api-template)

---
 
## `📜 CHRONICLES OF THE JOURNEY 📜`
 
```text
YEAR  ROLE                              COMPANY                           DURATION  TIMELINE
────  ────────────────────────────────  ───────────────────────────────   ────────  ──────────────────────────────
2025  Senior Software Engineer          Thomson Reuters (via SinQia)       ongoing  ███████████████████░░░░░░░░░░░
2024  Senior Software Engineer          FiServ (via Go2Win)                ongoing  ████████████████████████░░░░░░
2022  Senior Software Engineer          KIS Solutions                        24 mo  ██████████████████████████████
2021  Senior Software Engineer          Metal Toad                           14 mo  ██████████████████░░░░░░░░░░░░
2021  Tech Lead                         Opah IT / Banco BV                   15 mo  ███████████████████░░░░░░░░░░░
2020  Tech Lead                         Via Varejo (Casas Bahia)             11 mo  ██████████████░░░░░░░░░░░░░░░░
2019  Senior Software Engineer          UOL                                   7 mo  █████████░░░░░░░░░░░░░░░░░░░░░
2019  Senior Java Developer             CVC Corp                              6 mo  ████████░░░░░░░░░░░░░░░░░░░░░░
2019  Founder / Architect               HustleTech (own fiefdom)            78 mo*  ██████████████████████████████
2018  Software Developer                ITBAM                                 7 mo  █████████░░░░░░░░░░░░░░░░░░░░░
2016  Software Developer                Tap4 Mobile                          21 mo  ██████████████████████████░░░░
2016  Teacher / Instructor              Microlins                            18 mo  ██████████████████████░░░░░░░░
2016  Support Analyst (Rio Olympics)    ATOS                                  2 mo  ██░░░░░░░░░░░░░░░░░░░░░░░░░░░░
2014  IT Analyst                        Gibson, Inc.                         15 mo  ███████████████████░░░░░░░░░░░
2013  System Analyst                    LG CNS                               11 mo  ██████████████░░░░░░░░░░░░░░░░
2012  Innovation Analyst                Samsung Electronics                  12 mo  ███████████████░░░░░░░░░░░░░░░
2011  Systems Analysis Intern           Philips                              11 mo  ██████████████░░░░░░░░░░░░░░░░
2009  IT Support Intern                 Prefeitura de Manaus                 24 mo  ██████████████████████████████
 
* HustleTech is my own company — running in parallel to all roles above since 2019.
```

<details>
<summary>📖 <b>The full chronicle</b> — what was built at every hold</summary>

<br>

### Senior Software Engineer · Thomson Reuters via SinQia · `Feb 2025 — Now`
**Petrochemical Tax Systems** · Remote
Built **SCANC from scratch** — centralized fiscal obligation engine. Raw data import from Firebird, CDC + ETL in Python/Pandas, field-level versioning with full audit trail, Oracle PL/SQL SAFX tax rules. End-to-end processing in **under 12 seconds**. Event-driven workflows with AWS Lambda/SQS/SNS. Applied LLM tooling for architecture review and code generation.

### Senior Software Engineer · FiServ via Go2Win · `Oct 2024 — Now`
**Banking & Financial Compliance** · Remote
Built **AI-powered incident intelligence system** — LLM agents generate optimized Splunk queries, cross-reference curated incident history, surface past RCAs instantly. Within 3 months most recurring incidents resolve near-automatically.
Investigate full transaction stack end-to-end: Apigee → load balancer → NGINX → OpenShift/PCF → Java APIs → VMX mainframe. CI/CD via Jenkins + Harness, quality gates via SonarQube + Fortify.

### Senior Software Engineer · KIS Solutions · `Sep 2022 — Sep 2024`
**Cross-industry Software Delivery** · Washington, USA (Remote)
Team of 8 engineers across multiple client projects. Provided technical direction, reviewed PRs, enforced quality standards, supported architectural decisions alongside senior architects. Java, Python, Node.js, React, React Native (iOS + Android). Built CDC and ETL pipelines with Node.js and Next.js.

### Senior Software Engineer · Metal Toad · `Jun 2021 — Aug 2022`
**Multi-client Software Delivery** · Oregon, USA (Remote)
Delivered projects for multiple US clients across industries. Java, Python, React, C#, Node.js. Direct client work — requirements gathering, technical solution design, delivery across concurrent projects.

### Tech Lead · Opah IT (Banco BV) · `Apr 2021 — Jul 2022`
Java, Spring, Redis, PostgreSQL, MongoDB. Engineered financial systems, removed technical roadblocks, kept the team shipping.

### Senior Software Engineer · Banco BV · `Apr 2021 — Nov 2021`
Java, Spring Boot, Apache Camel. Built backend systems that survived traffic spikes and improved system reliability.

### Tech Lead · Via Varejo (Casas Bahia) · `May 2020 — Apr 2021`
Node.js, Java Spring, Azure, MongoDB, Redis, Oracle, DB2. Led teams through CI/CD overhaul, microservices migration with clean architecture, complex multi-database systems.

### Senior Software Engineer · UOL · `Oct 2019 — May 2020`
Java 8 + Kotlin, Quarkus, Apache Camel, TDD with Spock. Production systems that didn't catch fire.

### Senior Java Developer · CVC via Opah · `Mar 2019 — Sep 2019`
Java Spring microservices at scale. Kafka, ELK, Jenkins, Oracle, MongoDB. Travel industry traffic — bursty, ruthless, unforgiving.

### Software Developer · ITBAM · `Jul 2018 — Feb 2019`
Java Spring backend + Android mobile for a food delivery platform.

### Software Developer · Tap4 Mobile · `Sep 2016 — Jun 2018`
PHP, Node.js, Java, Android, Objective-C. Full-stack across web and mobile.

### Teacher/Instructor · Microlins · `Jun 2016 — Dec 2017`
Turned beginners into devs who could actually ship. *Every Master was once an Apprentice.*

### Support Analyst · Atos · `Jun 2016 — Aug 2016`
On-the-ground tech support for the **Rio 2016 Olympics**. Nothing exploded — *which, in retrospect, was the achievement*.

### IT Support Intern · Prefeitura de Manaus · `May 2009 — May 2011`
Where it all began. The first holocron was opened.

---

### 🎓 Training of the Order

**B.Sc. Computer Science** — Universidade Estácio, São Paulo
`Jan 2023 — Dec 2026` · Algorithms, data structures, OS, networks, software engineering. Research focus on AI/ML for practical applications.

### 🎤 Public Doctrine (Speaking)

- **2021** · *MongoDB: What Problems Does It Actually Solve?* — Universidade Estácio
- **2020** · *Best Practices in RESTful API Development with Node.js and Swagger* — Universidade Estácio

### 🌍 Tongues of the Realm

- 🇧🇷 **Portuguese** — Native
- 🇺🇸 **English** — Fluent (interviews, presentations, negotiations with US/EU teams)
- 🇪🇸 **Spanish** — Advanced (client work with Mexico, Argentina, Colombia)

</details>

---

## `📜 SCROLLS & TRANSMISSIONS 📜`

> 🩸 **The next phase of the doctrine: writing in public.**
> *Architecting LLM agents for production · Building profitable SaaS as a solo founder · The boring engineering decisions that actually matter*

<div align="center">

[![Medium](https://img.shields.io/badge/Medium-12100E?style=flat-square&logo=medium&logoColor=white)](https://medium.com/@napalm23zero)
[![Dev.to](https://img.shields.io/badge/Dev.to-0A0A0A?style=flat-square&logo=dev.to&logoColor=white)](https://dev.to/napalm23zero)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/dantas-rodrigo)

</div>

---

## `⚡ HOLOCRONS · BY THE NUMBERS ⚡`

```text
🩸  WHERE THE BLOOD IS SPILLED  ·  languages, last 5 years across all roles

   🟫 Java                  ████████████████░░░░░░░░░░░░░░   32.0%
   🐍 Python                ████████░░░░░░░░░░░░░░░░░░░░░░   17.0%
   🟦 TypeScript            ██████░░░░░░░░░░░░░░░░░░░░░░░░   13.0%
   🟨 JavaScript            ████░░░░░░░░░░░░░░░░░░░░░░░░░░    9.0%
   📊 SQL / PL/SQL          ████░░░░░░░░░░░░░░░░░░░░░░░░░░    9.0%
   🟪 Kotlin                ███░░░░░░░░░░░░░░░░░░░░░░░░░░░    6.0%
   🐹 Go                    ███░░░░░░░░░░░░░░░░░░░░░░░░░░░    5.0%
   🐚 Bash                  ██░░░░░░░░░░░░░░░░░░░░░░░░░░░░    4.0%
   🌀 Groovy / C#           ██░░░░░░░░░░░░░░░░░░░░░░░░░░░░    3.0%
   📝 Other                 █░░░░░░░░░░░░░░░░░░░░░░░░░░░░░    2.0%
```

```text
🌙  WHEN THE CODE BURNS  ·  commit distribution across the day

   🌅 Dawn      (04–08)    ████████░░░░░░░░░░░░░░░░░░░░░░   26.8%
   🌞 Morning   (08–12)    ████░░░░░░░░░░░░░░░░░░░░░░░░░░   13.1%
   🌆 Afternoon (12–18)    ██████░░░░░░░░░░░░░░░░░░░░░░░░   19.6%
   🌇 Evening   (18–22)    ████████░░░░░░░░░░░░░░░░░░░░░░   24.0%
   🌃 Night     (22–04)    ███████░░░░░░░░░░░░░░░░░░░░░░░   23.5%
```

```text
🖥️  WHERE THE THRONE STANDS  ·  operating systems

   🐧 Linux                ████████████████████░░░░░░░░░░   64.0%
   🪟 Windows              ██████░░░░░░░░░░░░░░░░░░░░░░░░   20.4%
   🍏 macOS                █████░░░░░░░░░░░░░░░░░░░░░░░░░   15.4%
```

```text
🩸  THE HONEST SINS  ·  less serious

   🥤 Coffee Intake         ██████████████████████████████   9,000 cups
   🎮 Gaming Breaks         ████████████████████████░░░░░░   8,500 hours
   💤 Sleep Deprivation     ██████████████░░░░░░░░░░░░░░░░   5,000 hours
   🐱 StackOverflow Visits  █████░░░░░░░░░░░░░░░░░░░░░░░░░     700 visits
   🤬 Raging at Bugs        ████░░░░░░░░░░░░░░░░░░░░░░░░░░     666 meltdowns
   🐛 Debugging Hell        ██░░░░░░░░░░░░░░░░░░░░░░░░░░░░     404 escapes
   💀 Accidental rm -rf /   █░░░░░░░░░░░░░░░░░░░░░░░░░░░░░       1 near-death exp
```

---

## `⚔ AN OFFER OF ALLIANCE ⚔`

<div align="center">

> *"Power, when properly forged, is indistinguishable from competence."*

### Open to three kinds of pact:

<table>
<tr>
<td align="center" width="33%">

### 🩸
**Co-founder**
Technical co-founder for an early-stage company solving a real problem. *Equity over salary, mission over ego.*

</td>
<td align="center" width="33%">

### ⚔️
**CTO / VP Engineering**
At a small-to-medium company ready to scale architecture and team. *I take the wheel — you keep the vision.*

</td>
<td align="center" width="33%">

### 🏛️
**Staff / Principal Engineer**
At a product-led company where I can shape technical direction. *I shape the system. The system shapes the future.*

</td>
</tr>
</table>

<br>

[![Email](https://img.shields.io/badge/Send_a_Transmission-rodrigo.dantas@hustletech.dev-9D2933?style=flat-square&logo=gmail&logoColor=white&labelColor=0a0606)](mailto:rodrigo.dantas@hustletech.dev)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-9D2933?style=flat-square&logo=linkedin&logoColor=white&labelColor=0a0606)](https://www.linkedin.com/in/dantas-rodrigo)
[![Book a Call](https://img.shields.io/badge/Summon_Me-Google_Calendar-9D2933?style=flat-square&logo=google-calendar&logoColor=white&labelColor=0a0606)](https://calendar.app.google/ovSnBAqvXtRztvsd7)
[![Twitter](https://img.shields.io/badge/Twitter-Follow-9D2933?style=flat-square&logo=twitter&logoColor=white&labelColor=0a0606)](https://twitter.com/napalm23zero)

<br>

<sub><i>If you would build an empire worth building — let us speak.</i></sub>

</div>
